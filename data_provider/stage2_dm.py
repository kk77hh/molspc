...
from data_provider.molecule_caption_dataset import MoleculeCaption, MoleculeCaption_double_pretrain,MoleculeCaption_double_value,MoleculeCaption_double_finetune,MoleculeCaption_universal
import re
...
class TrainCollater_double_pretrain:
    def __init__(self, tokenizer, text_max_len, mol_ph, mol_token_id):
        self.text_max_len = text_max_len
        self.tokenizer = tokenizer
        self.collater = Collater([], [])
    ...
    def __call__(self, batch):
        ...
        graphs1,graphs2,graphs0, texts,texts_opt ,smiles_prompt,smiles_prompt_opt= zip(*batch)
        graphs1 = self.collater(graphs1)
        ## deal with prompt
        smiles_prompt = [smiles_handler(p, self.mol_ph)[0] for p in smiles_prompt]
        smiles_prompt_opt = [smiles_handler(p, self.mol_ph)[0] for p in smiles_prompt_opt]
        ...
        smiles_prompt_tokens_opt = self.tokenizer(text=smiles_prompt_opt,
                                                truncation=False,
                                                padding='longest',
                                                add_special_tokens=True,
                                                return_tensors='pt',
                                                return_attention_mask=True)
        is_mol_token_opt = smiles_prompt_tokens_opt.input_ids == self.mol_token_id
        smiles_prompt_tokens_opt['is_mol_token'] = is_mol_token_opt
        ...
        text_tokens_opt = self.tokenizer(text=texts_opt,
                                     truncation=True,
                                     padding='longest',
                                     add_special_tokens=True,
                                     max_length=self.text_max_len,
        ...
        graphs0 = self.collater(graphs0)
        
       
        #在这里是真正转化成tokens
        return graphs1,graphs2,graphs0,smiles_prompt_tokens,smiles_prompt_tokens_opt  ,text_tokens,text_tokens_opt
    



    
class TrainCollater_double_finetune:
    def __init__(self, tokenizer, text_max_len, mol_ph, mol_token_id):
        self.text_max_len = text_max_len
        self.tokenizer = tokenizer
        self.collater = Collater([], [])
...
class InferenceCollater_double_pretrain:
    def __init__(self, tokenizer, text_max_len, mol_ph, mol_token_id):
        self.text_max_len = text_max_len
        self.tokenizer = tokenizer
        self.collater = Collater([], [])
    ...
    def __call__(self, batch):
        ...
        smiles_prompt_tokens_opt = self.tokenizer(smiles_prompt_opt,
                                       return_tensors='pt', 
                                    #    max_length=self.text_max_len, 
                                       padding='longest', 
                                       truncation=False, 
        ...
        return graphs1,graphs2,graphs0, smiles_prompt_tokens, smiles_prompt_tokens_opt,texts,texts_opt     
    




class InferenceCollater_double_finetune:
    def __init__(self, tokenizer, text_max_len, mol_ph, mol_token_id):
        self.text_max_len = text_max_len
        self.tokenizer = tokenizer
        self.collater = Collater([], [])
...
class Stage2DM_double_pretrain(LightningDataModule):
    def __init__(
        self,
        mode: str = 'pretrain',
        num_workers: int = 0,
        ...
        self.pretrain_dataset =  MoleculeCaption_double_pretrain(root, text_max_len, self.prompt)
        self.train_dataset =  MoleculeCaption_double_pretrain(root, text_max_len, self.prompt)
        self.val_dataset =  MoleculeCaption_double_pretrain('data/compare/valid/', text_max_len, self.prompt)
        self.test_dataset = MoleculeCaption_double_pretrain('data/compare/valid/', text_max_len, self.prompt)
        self.init_tokenizer(tokenizer)
        self.mol_ph_token = '<mol>' * self.args.num_query_token
    ...
    def train_dataloader(self):
        if self.mode == 'pretrain':
            loader = DataLoader(
                ...
                drop_last=True,
                persistent_workers=True,
                collate_fn=TrainCollater_double_pretrain(self.tokenizer, self.text_max_len, self.mol_ph_token, self.mol_token_id),
            )
        else:
            raise NotImplementedError
        return loader
    ...
    def val_dataloader(self):
        ...
        test_loader = DataLoader(
            ...
            collate_fn=InferenceCollater_double_pretrain(self.tokenizer, self.text_max_len, self.mol_ph_token, self.mol_token_id),
        )
        return [val_loader, test_loader]
...
class Stage2DM_double_finetune(LightningDataModule):
    def __init__(
        self,
        mode: str = 'pretrain',
        num_workers: int = 0,
        ...
        self.pretrain_dataset =  MoleculeCaption_double_finetune(root, text_max_len, self.prompt)
        self.train_dataset =  MoleculeCaption_double_finetune(root, text_max_len, self.prompt)
        self.val_dataset =  MoleculeCaption_double_finetune('data/opt/valid/', text_max_len, self.prompt)
        self.test_dataset = MoleculeCaption_double_finetune('data/opt/valid/', text_max_len, self.prompt)
        self.init_tokenizer(tokenizer)
        self.mol_ph_token = '<mol>' * self.args.num_query_token
    ...
    def train_dataloader(self):
        if self.mode == 'ft':
            loader = DataLoader(
                ...
                drop_last=True,
                persistent_workers=True,
                collate_fn=TrainCollater_double_finetune(self.tokenizer, self.text_max_len, self.mol_ph_token, self.mol_token_id),
            )
        else:
            raise NotImplementedError
        return loader
    ...
    def val_dataloader(self):
        ...
        test_loader = DataLoader(
            ...
            persistent_workers=True,
            #注意 在这里和下面我把测试集数据也都改了
            collate_fn=InferenceCollater_double_finetune(self.tokenizer, self.text_max_len, self.mol_ph_token, self.mol_token_id),
        )
        return [val_loader, test_loader]
...
from pytorch_lightning import LightningDataModule
from torch.utils.data import DataLoader
from data_provider.molecule_caption_dataset import InferenceDataset
#from data_provider.stage2_dm import InferenceCollater, InferenceCollater_double
from model.blip2_opt import smiles2data

class InferenceDM(LightningDataModule):
    def __init__(self, smiles, prompt, tokenizer, text_max_len, num_query_token):
        super().__init__()
        self.dataset = InferenceDataset(smiles, prompt, smiles2data)
        self.tokenizer = tokenizer
        self.text_max_len = text_max_len
        self.mol_ph_token = '<mol>' * num_query_token
        self.mol_token_id = tokenizer("<mol>", add_special_tokens=False).input_ids[0]
