...
import torch
from model.blip2_opt import Blip2OPT_pretrain, Blip2OPT_finetune
...
class Blip2Stage2_pretrain(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        if args.opt_model.find('galactica') >= 0:
            self.blip2opt = Blip2OPT_pretrain(args.bert_name, args.gin_num_layers, args.gin_hidden_dim, args.drop_ratio, args.tune_gnn, args.num_query_token, args.cross_attention_freq, args.llm_tune, args.peft_dir, args.opt_model, args.prompt, args)
    ...
    def save_predictions_valid(self, predictions, targets,epoch):
        assert len(predictions) == len(targets)
        print("----------------")
        print(len(predictions))
        print("----------------")
        #with open(os.path.join(self.logger.log_dir, 'predictions.txt'), 'w', encoding='utf8') as f:
        with open(os.path.join('valid_result/', str(epoch)+'_predictions.txt'), 'w', encoding='utf8') as f:
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx):
        if self.args.double == True :
            if (self.current_epoch+1) % self.caption_eval_epoch != 0:
                return 
            graphs1,graphs2, graphs0,prompt_tokens,_, texts, _ = batch
            ###============== Captioning Results ===================###
            samples = {'graphs1': graphs1, 'graphs2': graphs2,'prompt_tokens': prompt_tokens}
            predictions = self.blip2opt.generate(
                samples, 
    ...
    def validation_epoch_end(self, outputs):
        if self.current_epoch != 100000:
            ...
            if self.global_rank == 0:
                all_predictions = [i for ii in all_predictions for i in ii]
                all_targets = [i for ii in all_targets for i in ii]
                self.save_predictions_valid(all_predictions, all_targets, self.current_epoch)
...
class Blip2Stage2_finetune(pl.LightningModule):
    def on_save_checkpoint(self, checkpoint: Dict[str, Any]) -> None:
    ...
    def __init__(self, args):
        ...
        self.llm_tune = args.llm_tune
        if args.opt_model.find('galactica') >= 0:
            self.blip2opt = Blip2OPT_finetune(args.bert_name, args.gin_num_layers, args.gin_hidden_dim, args.drop_ratio, args.tune_gnn, args.num_query_token, args.cross_attention_freq, args.llm_tune, args.peft_dir, args.opt_model, args.prompt, args)
        elif args.opt_model.find('llama') >= 0 or args.opt_model.find('vicuna') >= 0:
            self.blip2opt = Blip2Llama(args.bert_name, args.gin_num_layers, args.gin_hidden_dim, args.drop_ratio, args.tune_gnn, args.num_query_token, args.cross_attention_freq, args.llm_tune, args.peft_dir, args.opt_model, args.prompt, args)
        else:
            raise NotImplementedError()
