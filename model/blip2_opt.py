...
class Blip2OPT_pretrain(Blip2Base):
    """
    ...
    def forward(self, batch):
        if self.args.double==False and self.args.solve == False and self.args.fangguangtuan == False and self.args.DDI == False:
            ...
            graphs1, graphs2,graphs0,prompt_tokens,prompt_tokens_opt, text_tokens, text_tokens_opt= batch
            ...
            mol_tokens = torch.cat([mol_tokens1,mol_tokens1], dim=1)
            
            empty_targets = torch.ones(prompt_tokens_opt.attention_mask.shape, dtype=torch.long).to(device).fill_(-100)
            targets = text_tokens_opt.input_ids.masked_fill(
                text_tokens_opt.input_ids == self.opt_tokenizer.pad_token_id, -100
            )
            targets = torch.cat([empty_targets, targets], dim=1)
            prompt_embeds = self.opt_model.get_input_embeddings()(prompt_tokens_opt.input_ids)
            prompt_embeds[prompt_tokens_opt.is_mol_token] = mol_tokens.flatten(0, 1)
            inputs_embeds = self.opt_model.get_input_embeddings()(text_tokens_opt.input_ids)
            inputs_embeds = torch.cat((prompt_embeds, inputs_embeds), dim=1)
            attention_mask = torch.cat([prompt_tokens_opt.attention_mask, text_tokens_opt.attention_mask], dim=1)
            ...
            loss = outputs.loss+outputs1.loss
            return {"loss": loss}
...
class Blip2OPT_finetune(Blip2Base):
    ...
    def generate(
        ...
        if self.args.double == True:
            ...
            prompt_embeds[prompt_tokens.is_mol_token] = torch.tensor(mol_tokens.flatten(0, 1),dtype=torch.bfloat16)
            ...
            output_text = [text.strip().split('.')[0] for text in output_text]
