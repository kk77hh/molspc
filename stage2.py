...
from data_provider.stage2_dm import Stage2DM,Stage2DM_double_finetune, Stage2DM_double_pretrain
from model.blip2_stage2 import Blip2Stage2_pretrain, Blip2Stage2_finetune, Blip2Stage2
...
def main(args):
    ...
    # model
    if args.mode in {'pretrain'}:
        if args.init_checkpoint:
            model = Blip2Stage2_pretrain.load_from_checkpoint(args.init_checkpoint, strict=False, args=args)
            print(f"loaded init checkpoint from {args.init_checkpoint}")
        elif args.stage2_path:
            model = Blip2Stage2_pretrain(args)
        ...
        if args.init_checkpoint:
            model = Blip2Stage2_finetune.load_from_checkpoint(args.init_checkpoint, strict=False, args=args)
            print(f"loaded init checkpoint from {args.init_checkpoint}")
        elif args.stage2_path:
            model = Blip2Stage2_finetune(args)
            ckpt = torch.load(args.stage2_path, map_location='cpu')
            model.load_state_dict(ckpt['state_dict'], strict=False)
            print(f"loaded stage2 model from {args.stage2_path}")
        elif args.stage1_path:
    ...
    # data
    if args.mode in {'pretrain'}:
        dm = Stage2DM_double_pretrain(args.mode, args.num_workers, args.batch_size, args.root, args.text_max_len, tokenizer, args)
    elif args.mode in {'ft', 'eval'}:
        dm = Stage2DM_double_finetune(args.mode, args.num_workers, args.batch_size, args.root, args.text_max_len, tokenizer, args)
